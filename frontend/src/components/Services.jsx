...
const Services = () => {
  ...
  const handleGetStarted = (message) => {
    ...
    window.open(`https://wa.me/919490901527?text=${encodedMessage}`, '_blank');
  };
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="text-center mb-16">
          <div className="inline-block px-4 py-2 bg-blue-600/10 border border-blue-500/20 rounded-full mb-4">
            <span className="text-sm text-blue-400 dark:text-blue-400 font-semibold uppercase tracking-wider">Services</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, index) => (
            <div
              key={index}
              className="group p-8 bg-white dark:bg-white/5 backdrop-blur-sm rounded-2xl border border-gray-200 dark:border-white/10 hover:border-blue-500/50 transition-all duration-400 hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/30 hover:-translate-y-2 flex flex-col"
            >
              <div className="mb-6 p-4 bg-gradient-to-br from-blue-600/20 to-purple-600/20 rounded-xl inline-block group-hover:scale-110 transition-transform duration-400">
                <div className="text-blue-500 dark:text-blue-400">{service.icon}</div>
              </div>
